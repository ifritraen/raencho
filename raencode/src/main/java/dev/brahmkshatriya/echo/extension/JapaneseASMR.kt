...
class JapaneseASMR : ExtensionClient, HomeFeedClient, AlbumClient, TrackClient, SearchFeedClient, LyricsClient {
    ...
    private fun parsePosts(html: String): List<Album> {
        ...
        // 1. Try standard list elements
        val posts = doc.select("li.site-archive-post, article, .post, .type-post")
        for (el in posts) {
            val titleEl = el.selectFirst("h2.entry-title a, h2 a, h3 a, .entry-title a, .post-title a") ?: continue
            val title = titleEl.text().trim()
            val href = titleEl.attr("href") ?: continue
        ...
        // 2. Fallback to parsing all links in the page where the path ends in a numeric ID
        if (albums.isEmpty()) {
    ...
    override suspend fun loadAlbum(album: Album): Album {
        return withContext(Dispatchers.IO) {
            ...
            val doc = Jsoup.parse(html)
            val contentEl = doc.selectFirst(".entry-content, p")
    ...
    override suspend fun loadTracks(album: Album): Feed<Track>? = PagedData.Single {
        withContext(Dispatchers.IO) {
            ...
            val doc = Jsoup.parse(html)
            val text = doc.selectFirst(".entry-content, p")?.text() ?: ""
            val rjCode = "RJ\\d{6,8}".toRegex().find(doc.html() + " " + text)?.value ?: album.extras["rj"] ?: ""
            ...
            for (player in players) {
                ...
                for (source in sources) {
                    ...
                    if (srcUrl.isNotEmpty() && (srcUrl.contains("weeab0") || srcUrl.contains(".mp3") || srcUrl.contains(".m4a") || srcUrl.contains(".m3u8"))) {
            ...
            // 4. Fallback to standard RJ Code .m3u8 structure
            ...
            baseToUrls.entries.forEachIndexed { index, entry ->
                ...
                val partTitle = if (suffix.isNotEmpty() && suffix != "1") suffix else "Main"
